---
title: "Predicting Home Values in San Francisco - MUSA507 Midterm"
...
options(repos = r)
setwd("~/Desktop/MUSA507/Midterm/Final_Data")
...
neighborhoods <- st_read("https://data.sfgov.org/api/geospatial/p5b7-5n3h?method=export&format=GeoJSON") %>%
  st_transform(4326)
SF_homes <- st_read("SF_homes_1.shp") %>%
  st_transform(4326)

Homes_landUse <- st_read("Homes_landUse/Homes_landUse.shp") %>%
  st_transform(4326)



# spatial join of homes to neighborhood boundaries
SF_homes_join <- st_join(SF_homes, neighborhoods)

#spatial join the land use file
...
#testing with median house value
...
# ---- fixing No Data ----
#clean up zeros
sale_jointable <- read_sf("points_join.shp")
...
SFcrimes <- read.csv("Police_Department_Incident_Reports__Historical_2017.csv")

#filter and test extent of each crime type to explain sale price
SFcrimes.sf <-
  SFcrimes %>%
...
#2.4 Feature Engineering crime data
...
#OLS model - relationship between sale price and living area
...
# ---- testing relationships amongst predictors ----
...
# getting the error for the test set
SF.test_2 <-
  SF.test_2 %>%
...
# are the home prices spatially autocorrelated
...
#is the error spatially autocorrelated?
coords.test <- 
...
In Figure 2.1, four indicators of interest are shown in a correlation scatterplot. Variables that appeared to have correlation with sale price were tested to be included in the model. 
```{r, echo=FALSE}
census_1 %>%
  ...
  facet_wrap(~Variable) +
  labs(title = "Fig. 2.1 Correlations between Sale Price and Selected Variables")
```
...
![](~/Desktop/MUSA507/Midterm/Final_Data/NumberofRooms.png)
